...
def update_stream():
    ...
    if stream_query == None:
        ...
        db.session.query(Stream).filter(Stream.stream_key==get_live['stream_key']).update({'title' : get_live['title'],'level' : get_live['level']})
        db.session.commit()
        ret = {
            'code' : 101,
...
def get_fans_list():
    ...
    if fans_list == None:
        ...
        return jsonify(fans=[e.serialize() for e in fans_list])
